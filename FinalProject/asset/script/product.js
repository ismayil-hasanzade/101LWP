...
fetch(`http://localhost:3000/Product/${id}`)
  ...
  .then((data) => {
    ...
    if (data.comments) {
      if (data.comments.review.length > 0) {
        for (let j = 0; j < data.comments.review.length; j++) {
          ...
          p.innerText = data.comments.review[j];
          span.innerText = data.comments.commentname[j];
...
submitbtn.addEventListener("click", async () => {
  ...
  if (nameinput.value === "" || email.value === "" || textarea.value === "") {
    ...
    await fetch(`http://localhost:3000/Product/${id}/`)
      ...
      .then((data) => {
        obj = data;
        const comments = obj.comments || {}; // Serverde olmayanda bosh obyekt yaradacaq

        comments.review = comments.review || []; //  Serverde olmayanda bosh arry yaradacaq
        comments.review.push(textarea.value);

        comments.commentname = comments.commentname || []; //  Serverde olmayanda bosh arry yaradacaq
        comments.commentname.push(nameinput.value);

        comments.email = comments.email || []; //  Serverde olmayanda bosh arry yaradacaq
        comments.email.push(email.value);
